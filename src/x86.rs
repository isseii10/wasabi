...
impl Idt {
    pub fn new(segment_selector: u16) -> Self {
        ...
        unsafe { asm!("lidt [rcx]", in("rcx") &params) }
        Self { entries }
    }
}
...
/// So is programmer's responsibility to ensure that at most one instance of the reference exist at every moment.
...
/// # Safety
/// This function modifies the page table as callback does, so anything bad can happen if there are some mistakes.
...
pub fn disable_cache<T: Sized>(io_box: &IoBox<T>) {
    ...
    unsafe {
        with_current_page_table(|pt| {
            pt.create_mapping(vstart, vend, vstart, PageAttr::ReadWriteIo).expect("Failed to create_mapping")
        });
    }
}
