...
impl Pci {
    ...
    pub fn write_regiter_u64(&self, bdf: BusDeviceFunction, byte_offset: usize, data: u64) -> Result<()> {
    ...
    pub fn set_command_and_status_flags(&self, bdf: BusDeviceFunction, flags: u32) ->Result<()>{
        ...
        self.write_register_u32(bdf, 0x04, /* Command and status */ flags | cmd_and_status)
    }
...
impl fmt::Debug for BarMem64 {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        write!(f,"BarMem64[{:#018X}..{:#018X}]", self.addr as u64, self.addr as u64 + self.size())
    }
