...
impl DeviceContextBaseAddressArray {
    pub fn new(scratchpad_uffers: ScratchpadBuffers) -> Self {
        let mut inner = RawDeviceContextBaseAddressArray::new();
        inner.scratchpad_table_ptr = scratchpad_uffers.table.as_ref().as_ptr();
        Self {
            _inner: Box::pin(inner),
            _context: unsafe { MaybeUninit::zeroed().assume_init() },
            _scratchpad_buffers: scratchpad_uffers,
        }
    }
}
